	...
	$.fn.responsiveSlides = function(options){
		...
		$('img',$('.tgtimg',_this)).onImagesLoad(function(){
			...
			if(p.active){
				...
				pos+=(p.position.substring(2,3)=='L')?'left:'+15+'%; ':(p.position.substring(2,3)=='R')?'right:'+15+'%; ':'left:50%; ';
				var elm='';
				
				$('img',$('.tgtimg',_this)).each(function(i,e){elm+='<div'+((p.dotStyle=='')?' style="float:left; height:25px; width:25px; background:#da1921; margin-left:2px; cursor:pointer"':' class="'+p.dotStyle+'"')+'></div>';});
				...
				$('div',$('.pagination',_this)).mouseover(function(){
					if(p.dotStyleHover!=''){$(this).addClass(p.dotStyleHover);}else{$(this).css('background','#cccccb');}
				}).mouseout(function(){
					if(p.dotStyleHover!=''){$(this).removeClass(p.dotStyleHover);}else{$(this).css('background','#da1921');}
				}).click(function(){
					_this.RS_Stop();
